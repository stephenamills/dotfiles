...
def required_lesson_catalog(prompt: str) -> list[tuple[str | None, str]]:
    match = re.search(
        r"(?ms)===== REQUIRED LESSON CATALOG CONTRACT =====\n.*?\n"
    ...
    for line in match.group(1).splitlines():
        if line.startswith("### "):
            ...
            entries.append((group, line[2:].strip()))
    return entries


def candidate(prompt: str = "") -> str:
    words = " ".join(
        f"concept{index} is explained accurately and clearly"
        for index in range(1, 90)
    )
    catalog = required_lesson_catalog(prompt)
    catalog_section = ""
    if catalog:
        rows = "\n".join(
            f"| {index} | {label} | Observe the supported technique and decision cues. |"
            for index, (_, label) in enumerate(catalog, 1)
        )
        catalog_section = (
    ...
    overview_heading = "## 1. Overview" if catalog else "## Overview"
    concepts_heading = "## 3. Key Concepts" if catalog else "## Key Concepts"
    review_heading = "## 4. Review Questions" if catalog else "## Review Questions"
    return (
        "# Lesson Study Guide\n\n"
        f"{overview_heading}\n\n"
        f"{words}.\n\n"
...
def course_map_candidate(prompt: str) -> str:
    ...
    catalog = required_lesson_catalog(prompt)
    catalog_groups: list[str] = []
    active_group: str | None = None
    for group, label in catalog:
        if group != active_group:
            active_group = group
            catalog_groups.extend([f"### {group}", ""])
        catalog_groups.append(f"- {label}")
    ordered_catalog = "\n".join(catalog_groups)
    ...
    for index, heading in enumerate(headings, 1):
        sections.extend([f"## {index}. {heading}", ""])
        if heading in {"Ordered Chapter Path", "Ordered Topic Path"}:
            sections.extend([ordered_links, "", ordered_catalog, ""])
        elif "Architecture and Dependencies" in heading:
            sections.extend(
                [
                    "```mermaid",
                    "flowchart LR",
        ...
        elif "Workflow and Sensory Decision Gates" in heading:
            sections.extend(
                [
                    "```mermaid",
                    "flowchart TD",
