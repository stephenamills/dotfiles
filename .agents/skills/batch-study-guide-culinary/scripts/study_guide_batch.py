...
DEFAULT_WHOLE_COURSE_MAP_PROMPT_PATH = SKILL_ROOT / "references" / "default-whole-course-map-prompt.md"
VALID_REASONING_EFFORTS = {"none", "low", "medium", "high", "xhigh", "max"}
VALID_VERBOSITY_LEVELS = {"low", "medium", "high"}
UNIT_KINDS = {"transcript", "course_map"}
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "generation": {
        "include_existing_target_context": True,
    },
    "prompts": {
        "root": None,
        "per_unit": {},
...
def validate_config(root: Path, config: dict[str, Any]) -> None:
    ...
    if present:
        raise BatchError(
            "transcript-only configuration rejects asset fields: " + ", ".join(present)
        )
    by_kind = config.get("prompts", {}).get("by_kind", {})
    rejected_prompt_kinds = sorted({"pdf", "spreadsheet", "workbook"}.intersection(by_kind))
    if rejected_prompt_kinds:
        raise BatchError(
            "prompts.by_kind rejects non-transcript kinds: " + ", ".join(rejected_prompt_kinds)
        )
    ...
    lesson_catalog = config.get("lesson_catalog", {})
    if not isinstance(lesson_catalog, dict):
        raise BatchError("lesson_catalog must be an object")
    for key in ("enabled", "topic_maps", "whole_course"):
        if not isinstance(lesson_catalog.get(key), bool):
            raise BatchError(f"lesson_catalog.{key} must be true or false")
    if lesson_catalog.get("label_style") != "readable":
        raise BatchError("lesson_catalog.label_style must be readable")
    generation = config.get("generation", {})
    if not isinstance(generation, dict):
        raise BatchError("generation must be an object")
    if not isinstance(generation.get("include_existing_target_context"), bool):
        raise BatchError("generation.include_existing_target_context must be true or false")
...
def readable_lesson_label(source: str | Path) -> str:
    """Derive a stable, human-readable lesson label from a transcript filename."""
    ...
    if len(parts) >= 3 and re.fullmatch(r"\d+", parts[0]):
        numbered = next(
            (index for index, part in enumerate(parts[1:], 1) if re.match(r"^\d+\.", part)),
            None,
        )
        if numbered is not None:
            parts = parts[numbered:]
        elif len(parts) >= 4 and parts[1].casefold() == "tips":
            parts = parts[3:]
        else:
            parts = parts[2:]
...
def relative_topic_lesson_label(chapter_title: str, label: str) -> str:
    """Remove chapter text already supplied by the surrounding topic-map H3."""
    chapter_name = re.split(r"\s+[-–—]\s+", chapter_title, maxsplit=1)[-1].strip()
    value = re.sub(r"^\d+(?:\.\d+)*[.)]?\s+", "", label).strip()
    parts = [part.strip() for part in value.split("—", 1)]
    if len(parts) == 2 and parts[0].casefold() == chapter_name.casefold():
        return parts[1]
    return value
...
def prompt_selection(
    ...
    if configured_unit:
        ...
        if kind == "transcript":
            selected = DEFAULT_PROMPT_PATH.resolve()
        elif unit_id == "course-map-whole-course":
            selected = DEFAULT_WHOLE_COURSE_MAP_PROMPT_PATH.resolve()
        else:
            selected = DEFAULT_TOPIC_MAP_PROMPT_PATH.resolve()
...
def build_units(root: Path, config: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    blockers: list[str] = []
    if not transcripts:
        blockers.append("no transcript units were found")
    ...
    for ordinal, group in enumerate(raw_groups, 1):
        ...
        lesson_labels = (
            [readable_lesson_label(path) for path in group.get("transcript_sources", [])]
            if config["lesson_catalog"]["enabled"] and kind == "transcript"
            else []
        )
        units.append(
            {
                ...
                "source_types": {rel: "transcript" for rel in source_rels},
                ...
                "include_existing_target_context": config["generation"][
                    "include_existing_target_context"
                ],
            }
        )
                ...
                [
                    {
                        ...
                        "target": dependency["target"],
                        "labels": [
                            relative_topic_lesson_label(dependency["title"], label)
                            for label in dependency.get("lesson_labels", [])
                        ],
                    }
                    for dependency in dependencies
                ]
                if config["lesson_catalog"]["enabled"]
                and config["lesson_catalog"]["topic_maps"]
                else []
            )
            topic_map_unit = {
                    "id": unit_id,
                    ...
                    "lesson_labels": [
                        label
                        for group in lesson_catalog_groups
                        for label in group["labels"]
                    ],
                    "lesson_catalog_groups": lesson_catalog_groups,
                    "include_existing_target_context": config["generation"][
                        "include_existing_target_context"
                    ],
                }
                    ...
                    [
                        {
                            "title": dependency["title"],
                            "target": dependency["target"],
                            "labels": dependency.get("lesson_labels", []),
                        }
                        for dependency in dependencies
                    ]
                    if config["lesson_catalog"]["enabled"]
                    and config["lesson_catalog"]["whole_course"]
                    else []
                )
                units.append(
                    {
...
def mapping_hash(units: list[dict[str, Any]], config: dict[str, Any]) -> str:
    material = {
        "units": [
            {
                ...
                "lesson_labels": unit.get("lesson_labels", []),
                "lesson_catalog_groups": unit.get("lesson_catalog_groups", []),
                "include_existing_target_context": unit.get(
                    "include_existing_target_context", True
                ),
            }
            for unit in units
        ...
        "lesson_catalog": config["lesson_catalog"],
...
def make_contract(
    ...
    *,
    ...
    return {
        ...
        # only wall-clock stop is the immutable run deadline below; progress
...
def approve_plan(store: Store, plan: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    contract = make_contract(
        ...
        model_reasoning_effort=model_reasoning_effort,
        model_verbosity=model_verbosity,
        install_immediately=bool(getattr(args, "install_immediately", False)),
    )
    approval_material = {
        "plan_id": plan["id"],
        "mapping_hash": plan["mapping_hash"],
...
def render_approval_report(path: Path, approval: dict[str, Any], calibration: dict[str, Any], unit_count: int) -> None:
    ...
    lines = [
        ...
        "Per-call timeout: disabled (run hard deadline governs)",
...
def disabled_worker_skill_args() -> list[str]:
    ...
    candidates = [
        Path.home() / ".agents" / "skills" / "batch-study-guide-culinary",
        Path.home() / ".codex" / "skills" / "batch-study-guide-culinary",
    ]
...
def extracted_source_text(path: Path, kind: str, max_chars: int, encoding: str) -> str:
    if kind in {"transcript", "markdown"}:
        value = path.read_text(encoding=encoding)
        return value if len(value) <= max_chars else value[:max_chars]
    raise BatchError(f"Unsupported transcript-only source kind: {kind}")
...
def copy_stage_inputs(
    ...
    for index, relative in enumerate(unit["sources"], 1):
        ...
        kind = unit.get("source_types", {}).get(relative, "transcript")
        if override is None:
        ...
        if unit.get("id") == "course-map-whole-course":
            extracted = compact_whole_course_source(extracted)
    ...
    target = root / unit["target"]
    if unit.get("include_existing_target_context", True) and target.is_file():
...
def stage_instruction(unit: dict[str, Any]) -> str:
    ...
    common = [
        ...
        "Choose the Mermaid type that matches the relationship: use `mindmap` for culinary hierarchy, `flowchart` for workflows and sensory decisions, and `stateDiagram-v2` for transformations, holding, recovery, and feedback cycles.",
        "Never use a diagram to depict exact knife grip, guide-hand position, or other physical hand positioning that could be misrepresented.",
        ...
        "Preserve every supported measurement, ratio, time, and temperature exactly. Explain its function and scaling only where supported, and never turn lesson-specific guidance into a universal rule.",
        "Prefer plain Markdown for temperatures, ratios, percentages, and arithmetic. Do not introduce KaTeX unless the material genuinely requires mathematical notation.",
    ]
    if kind == "course_map":
        if unit.get("id") == "course-map-whole-course":
            common.extend(
                [
                    "This unit is the in-depth whole-course map. Use only the complete ordered set of supplied topic course maps as source material. Do not infer from, request, or reach back to transcripts or study chapters.",
                    ...
                    "Cover every supplied topic map in the ordered path and detailed mastery table, then synthesize technique dependencies, transformations, timing, sensory gates, service, troubleshooting, and recovery across the full course.",
            ...
            common.extend(
                [
                    ...
                    "Cover every supplied chapter in the ordered path and detailed mastery table, then synthesize technique dependencies, ingredients, tools, mise en place, transformations, timing, sensory gates, service, troubleshooting, and recovery across chapter boundaries.",
        ...
        common.extend(
            [
                "Ground the chapter exclusively in the supplied transcripts. Do not browse, add external facts, invent recipes, or introduce unsupported safety or temperature rules.",
                "Silently repair only unambiguous transcription artifacts while preserving culinary terminology, qualifications, measurements, and sequence.",
                "Teach through the applicable dimensions: purpose, mise en place, tools, ingredients, preparation, physical technique, culinary transformations, heat/moisture/fat control, timing, sensory cues, decision points, doneness, troubleshooting, recovery, holding, storage, service, and practice.",
                "End with practical mastery checks and answered retrieval or scenario questions that emphasize observable cues and corrective decisions.",
            ]
        )
    common.append(
...
def direct_stage_prompt(unit: dict[str, Any], stage_dir: Path, correction: str | None = None) -> str:
    ...
    lesson_labels = unit.get("lesson_labels", [])
    if lesson_labels:
        if unit.get("kind") == "transcript":
            catalog_lines = [f"- {label}" for label in lesson_labels]
            catalog_instruction = (
                "Add an early numbered H2 titled exactly `Original Video Lessons`. "
                "Within that section, include one Markdown table row for every required lesson "
                "name in the exact order below. Use readable lesson names only: do not add media "
                "paths, links, transcript paths, filenames, or file extensions. Include a concise "
                "transcript-grounded `Watch for` cue for each row. Keep the main chapter integrated "
                "by culinary concept and workflow rather than organizing it as source summaries. "
                "Name the relevant video lesson again in the teaching body where it materially "
                "helps the learner reconnect instruction to the demonstration."
            )
        else:
            catalog_lines = []
            for group in unit.get("lesson_catalog_groups", []):
                catalog_lines.append(f"### {group['title']}")
                catalog_lines.extend(f"- {label}" for label in group["labels"])
            catalog_instruction = (
                "Inside `## 2. Ordered Chapter Path`, preserve the required chapter links and add "
                "an H3 for every chapter followed by all of its required readable video lesson "
                "names in the exact order below. The lesson names are deliberately relative to "
                "their H3 chapter heading; do not repeat the chapter title or its numeric prefix "
                "inside each lesson bullet. Do not add media paths, media links, transcript "
                "paths, filenames, or file extensions. This catalog is navigation inside an "
                "otherwise in-depth teaching synthesis; do not turn the map into an index."
            )
        sections.extend(
            [
                "\n===== REQUIRED LESSON CATALOG CONTRACT =====\n",
                catalog_instruction,
                "\n",
                "\n".join(catalog_lines),
                "\n===== END REQUIRED LESSON CATALOG CONTRACT =====\n",
            ]
        )
    if unit.get("kind") == "course_map":
...
SOURCE_ATTRIBUTION_ACTOR = r"(?:pdf|source|sources|transcript|instructor|lesson|course|document|guide)"
...
def normalize_heading_numbering_bytes(
    ...
    """Apply the deterministic heading repair before model retry/repair paths.

    Numbered H3 headings are only meaningful for learner-work sequences. Models
    sometimes number ordinary teaching subheadings (for example, ``### 2.
    Sizzle-and-release scenario``). Strip that decorative prefix locally rather
    than spending another generation call; H2 numbering is never changed.
    """
...
TOPIC_MAP_REQUIRED_SECTIONS = (
    "Topic Thesis and Learning Outcomes",
    "Ordered Chapter Path",
    "Technique Architecture and Dependencies",
    "Chapter-by-Chapter Learning and Mastery",
    "Ingredients, Tools, and Mise en Place",
    "Culinary Principles and Transformations",
    "Operating Workflow and Sensory Decision Gates",
    "Cross-Chapter Integration, Timing, and Service",
    "Troubleshooting, Recovery, and Safety",
    "Cumulative Kitchen Application",
    "Mastery Checklist",
...
WHOLE_COURSE_MAP_REQUIRED_SECTIONS = (
    ...
    "Ordered Topic Path",
    "Whole-Course Technique Architecture and Dependencies",
    "Topic-by-Topic Learning and Mastery",
    "Ingredients, Tools, and Mise en Place Across the Course",
    "Cross-Topic Culinary Principles and Transformations",
    "End-to-End Kitchen Workflow and Sensory Decision Gates",
    "Cross-Topic Technique Transfer, Timing, and Service",
    "Whole-Course Troubleshooting, Recovery, and Safety",
    "Cumulative Whole-Course Kitchen Application",
    "Whole-Course Mastery Checklist",
    "Whole-Course Retrieval Questions and Answers",
    "Whole-Course Spaced Practice Plan",
)
...
def numbered_h2_section(text: str, title: str) -> str | None:
    match = re.search(
...
def lesson_catalog_heading_key(value: str) -> str:
    """Normalize catalog group headings for harmless Markdown punctuation/numbering."""
...
def shorten_topic_map_catalog_labels(text: str) -> str:
    """Remove lesson prefixes already stated by each Ordered Chapter Path H3."""
    lines = text.splitlines(keepends=True)
    in_ordered_path = False
    chapter_name: str | None = None
    for index, line in enumerate(lines):
        if re.match(r"^##\s+\d+\.\s+Ordered Chapter Path\s*$", line.rstrip("\r\n")):
            in_ordered_path = True
            chapter_name = None
            continue
        if in_ordered_path and re.match(r"^##\s+", line):
            in_ordered_path = False
            chapter_name = None
            continue
        if not in_ordered_path:
            continue
        heading = re.match(r"^###\s+(.+?)\s*$", line.rstrip("\r\n"))
        if heading:
            visible = heading.group(1)
            link = re.match(r"^\[(.+?)\]\(.+\)$", visible)
            if link:
                visible = link.group(1)
            chapter_name = re.split(r"\s+[-–—]\s+", visible, maxsplit=1)[-1].strip()
            continue
        if not chapter_name:
            continue
        ending = "\n" if line.endswith("\n") else ""
        body = line.rstrip("\r\n")
        match = re.match(
            rf"^(\s*-\s+)(?:\d+(?:\.\d+)*[.)]?\s+)?{re.escape(chapter_name)}\s+—\s+(.+)$",
            body,
            re.I,
        )
        if match:
...
def normalize_installed_map_catalogs(root: Path) -> list[str]:
    config, _ = load_config(root)
    output_root = path_within(root, config["output_root"], "output root")
    map_root = output_root / config["course_maps"].get("output_folder", "0 Course Maps")
    changed: list[str] = []
    if not map_root.is_dir():
        return changed
    for path in sorted(map_root.glob("*.md")):
        original = path.read_text(encoding="utf-8")
        normalized = shorten_topic_map_catalog_labels(original)
...
def validate_lesson_catalog(
    text: str, unit: dict[str, Any]
) -> tuple[bool, str, str]:
    labels = list(unit.get("lesson_labels", []))
    if not labels:
        return True, "success", "lesson catalog not configured"
    if unit.get("kind") == "transcript":
        heading = "Original Video Lessons"
    elif unit.get("id") != "course-map-whole-course":
        heading = "Ordered Chapter Path"
    else:
        return True, "success", "whole-course lesson catalog not configured"
    section = numbered_h2_section(text, heading)
    if section is None:
        return False, "lesson_catalog", f"candidate lacks the required {heading!r} section"
    if unit.get("kind") == "course_map" and shorten_topic_map_catalog_labels(text) != text:
        return False, "lesson_catalog", (
            "'Ordered Chapter Path' repeats a chapter title or numeric chapter prefix "
            "inside its relative lesson bullets"
        )
    cursor = 0
    for label in labels:
        ...
        if match is None:
            return False, "lesson_catalog", (
                f"{heading!r} lacks required lesson name in source order: {label}"
            )
        cursor = match.end()
    # A chapter/map may repeat a lesson name in a heading, bullet, or cue. The
    # contract requires ordered findability, not an artificial global count.
    if unit.get("kind") == "transcript" and markdown_local_link_targets(section, "."):
        return False, "lesson_catalog", (
            "'Original Video Lessons' must use readable names without file or media links"
        )
    section_h3_keys = {
        lesson_catalog_heading_key(match.group(1))
        for match in re.finditer(r"(?m)^###\s+(.+?)\s*$", section)
    }
    for group in unit.get("lesson_catalog_groups", []):
        if lesson_catalog_heading_key(group["title"]) not in section_h3_keys:
            return False, "lesson_catalog", (
                f"'Ordered Chapter Path' lacks required chapter catalog heading: {group['title']}"
            )
    return True, "success", f"validated {len(labels)} ordered lesson name(s)"
...
def validate_unit_candidate_bytes(
    ...
    text = value.decode("utf-8")
    catalog_valid, catalog_category, catalog_detail = validate_lesson_catalog(text, unit)
    if not catalog_valid:
        return catalog_valid, catalog_category, catalog_detail
    if unit.get("kind") != "course_map":
        return True, "success", f"{detail}; {catalog_detail}"
    ...
    return True, "success", (
        ...
        f"{mermaid_count} Mermaid diagram(s), {len(unit['sources'])} chapter link(s), "
        f"{len(unit.get('lesson_labels', []))} lesson name(s); informational only)"
    )
...
def deterministic_source_attribution_repair(value: bytes) -> bytes:
    """Strip unambiguous provenance wrappers without rewriting culinary content."""
    text = value.decode("utf-8")
...
"""
    ...
    def dispatch_wave(
        ...
        max_runtime = max(1, math.ceil(float(self.contract["deadline_seconds"])))
    ...
    def invoke_legacy_once(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix=f"study-guide-{self.run_id}-{unit['id']}-") as temporary:
            ...
            # Do not terminate an individual call early; only the run's hard
            # deadline may stop it.
            timeout_seconds = max(
                0.1,
                (parse_iso(self.run_row()["deadline_at"]) - dt.datetime.now(dt.timezone.utc)).total_seconds(),
            )
            with event_log_path.open("wb") as event_log, stderr_path.open("wb") as stderr_log:
                process = subprocess.Popen(
                    command,
...
preserves all culinary meaning, measurements, qualifications, Markdown structure, links, table
columns, and ordering, and removes only the stated unwanted wrapper. Otherwise return
`REJECTED: ` followed by one concise reason. Do not rewrite the document. Do not call tools.
...
"""
    ...
    def _process_direct_rows(self, rows: Sequence[sqlite3.Row]) -> None:
        """Generate independent course maps without a nested V2 dispatcher."""
        def one(row: sqlite3.Row) -> tuple[str, str | None]:
            ...
            try:
                ...
                detail = f"{type(exc).__name__}: {exc}"
                self.set_unit(unit_id, "failed", detail=detail, completed_at=now_iso(),
                              lease_owner=None, lease_until=None, heartbeat_at=now_iso())
                append_event(self.store, self.run_id, {"type": "unit_exception",
                    "unit_id": unit_id, "detail": detail, "traceback": traceback.format_exc()})
                return unit_id, detail

        for row in rows:
            self.set_unit(row["unit_id"], "generating", lease_owner=f"{self.worker_tag}-direct",
                          heartbeat_at=now_iso(), started_at=row["started_at"] or now_iso(),
                          detail="direct course-map generation")
        with ThreadPoolExecutor(max_workers=min(len(rows), int(self.contract["workers"]))) as pool:
            futures = [pool.submit(one, row) for row in rows]
            for future in as_completed(futures):
                unit_id, detail = future.result()
                self.progress(f"{unit_id} · direct map {'failed: ' + detail if detail else 'approved'}")
                export_status(self.store, self.run_id)
                if self.stop_reason():
                    self.local_stop.set()
                    return
...
def install_approved_preview(
    ...
    """Expose a validated new target while retaining its promotion candidate."""
    ...
    if target.is_file():
        if sha256_file(target) != candidate_hash:
            if original_hash is None or sha256_file(target) != original_hash:
...
def add_common_root(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Folder containing transcripts and batch state",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan_parser = subparsers.add_parser(
        "plan",
        help="Discover transcript study chapters and course-map units without model calls",
    )
    ...
    preview_parser = subparsers.add_parser(
        "install-previews",
        help="Install validated course maps from an active run while retaining promotion candidates",
    )
    preview_parser.add_argument("run_id")
    add_common_root(preview_parser)

    normalize_maps_parser = subparsers.add_parser(
        "normalize-map-catalogs",
        help="Remove chapter-title repetition from installed topic-map lesson catalogs",
    )
    add_common_root(normalize_maps_parser)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "install-previews":
            ...
            return 0
        if args.command == "normalize-map-catalogs":
            print(json.dumps(normalize_installed_map_catalogs(root), indent=2))
            return 0
