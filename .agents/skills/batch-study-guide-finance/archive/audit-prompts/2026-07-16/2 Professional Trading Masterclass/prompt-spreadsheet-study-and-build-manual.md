...
Transform exactly one unique primary Microsoft Excel workbook (`.xlsx`, `.xlsm`, or `.xls`) and one or more explicitly corresponding lesson transcripts into one comprehensive Markdown study-and-build manual. The manual must both explain the supplied workbook and teach the learner to reproduce its intentional, functional worksheet design from a new blank workbook in the current Microsoft 365 version of Excel for Mac.
...
- one unique primary `.xlsx`, `.xlsm`, or `.xls` workbook, with every byte-identical source location identified;
...
1. Confirm that exactly one unique primary workbook and at least one explicitly corresponding transcript are present and readable.
2. Compute a SHA-256 hash for every supplied workbook path. Collapse only byte-identical copies into the single primary source and record all original paths. If same-named copies have different hashes, stop and require an explicit choice rather than silently selecting or merging them. Record hashes, duplicate-path evidence, style-record counts, and inspection-tooling details only in the audit files; never place them in the manual body, which may state only learner-relevant boundary facts such as which lessons distribute the workbook and which features are absent.
...
1. exactly one unique primary workbook and all explicitly corresponding transcripts were used, all duplicate workbook paths and hashes were recorded, and the workbook's actual format was identified from its contents;
